...
BUFFER_SIZE = int(1e6)        #number of transitions we are gonna store before rewriting old ones
MIN_REPLAY_SIZE = 50000          #minimum number of transitions required in buffer before we calculate the gradients and training
EPSILON_START = 1.0             #starting value of the epsilon(epsilon-greedy for exploration)
EPSILON_END = 0.1              #ending value of the epsilon    
EPSILON_DECAY = int(1e6)          #decay time steps of epsilon
NUM_ENVS = 4
TARGET_UPDATE_FREQ = 10000 // NUM_ENVS      #number of steps where we set the target parameters equal to online parameters
LR = 5e-5
    ...
    def act(self,obs, epsilon):
        ...
        for i in range(len(actions)):
            ...
            if rnd_sample <= epsilon:
                actions[i]: random.rand(0, self.num_actions - 1)
...
for step in itertools.count():
    ...
    if step % LOG_INTERVAL == 0:
        ...
        print()
        print('Step', step)
        print('Avg Rew', np.mean(rew_mean))
